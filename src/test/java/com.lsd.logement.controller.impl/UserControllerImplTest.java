...
public class UserControllerImplTest {
    ...
    public void getById() throws Exception {
        ...
        Mockito.when(userService.findById(ArgumentMatchers.anyInteger())).thenReturn(java.util.Optional.of(UserBuilder.getEntity()));
    ...
    public void update() throws Exception {
        Mockito.when(userMapper.asEntity(ArgumentMatchers.any())).thenReturn(UserBuilder.getEntity());
        Mockito.when(userService.update(ArgumentMatchers.any(), ArgumentMatchers.anyInteger())).thenReturn(UserBuilder.getEntity());
        ...
        mockMvc.perform(
                MockMvcRequestBuilders.put(ENDPOINT_URL + "/1")
                        ...
                        .content(CustomUtils.asJsonString(UserBuilder.getDTO())))
                .andExpect(MockMvcResultMatchers.status().isOk());
        Mockito.verify(userService, Mockito.times(1)).update(ArgumentMatchers.any(User.class), ArgumentMatchers.anyInteger());
        Mockito.verifyNoMoreInteractions(userService);
    }

    @Test
    public void delete() throws Exception {
        Mockito.doNothing().when(userService).deleteById(ArgumentMatchers.anyInteger());
        mockMvc.perform(
                MockMvcRequestBuilders.delete(ENDPOINT_URL + "/1"))
                .andExpect(MockMvcResultMatchers.status().isOk());
        Mockito.verify(userService, Mockito.times(1)).deleteById(Mockito.anyInteger());
        Mockito.verifyNoMoreInteractions(userService);
    }